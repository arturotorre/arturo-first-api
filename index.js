...
app.patch("/jokes/:id", (req,res) =>{
  ...
  jokes.find((joke) => {
    if(joke.id === id){
      joke.jokeText = req.body.text || foundJoke.jokeText;
      joke.jokeType = req.body.type || foundJoke.jokeType;
    }
  });
  res.json(foundJoke);
});
...
app.delete("/jokes/:id", (req,res) =>{
  const id = parseInt(req.params.id);
  jokes.splice(id-1);
  res.json("Joke deleted succesfully!");
});
...
app.delete("/all", (req,res) =>{
  ...
  if(masterKey === key){
    ...
    res
    .status(404)
    .json({ error: `Key provided ${key} is not correct, please try again.`})
  }
});
